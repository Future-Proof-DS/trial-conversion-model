...
def pull_newest_cohort() -> Path:
    ...
    Filenames carry the cohort date, so newest is the lexical maximum.
    ...
    keys = [
        o["Key"] for o in listing.get("Contents", []) if o["Key"].endswith(".parquet")
    ]
    if not keys:
        raise FileNotFoundError("no cohort files in the course bucket yet")
    key = max(keys)
